...
final class SslUtils {
     ...
     */
    static final int GMSSL_PROTOCOL_VERSION = 0x0101;
    ...
    static {
        ...
        defaultCiphers.add("TLCP_SM2-WITH-SMS4-SM3");
    ...
    /**
     * Returns {@code true} if the JDK itself supports TLSv1.3, {@code false}
     * otherwise.
     */
    ...
    /**
     * Returns {@code true} if the JDK itself supports TLSv1.3 and enabled it by
    ...
    /**
     * Add elements from {@code names} into {@code enabled} if they are in
     * {@code supported}.
     */
    ...
    /**
     * Converts the given exception to a {@link SSLHandshakeException}, if it isn't
     * already.
     */
    ...
    /**
     * Return how much bytes can be read out of the encrypted data. Be aware that
     * this method will not increase
     * the readerIndex of the given {@link ByteBuf}.
     *
     * @param buffer
     *               The {@link ByteBuf} to read from. Be aware that it must have at
     *               least
     ...
     *         {@link #SslUtils#NOT_ENOUGH_DATA} if not enough data is present in
     *         the
     *         {@link ByteBuf}. This will return {@link SslUtils#NOT_ENCRYPTED} if
     *         the given {@link ByteBuf} is not encrypted at all.
     * @throws IllegalArgumentException
     *                                  Is thrown if the given {@link ByteBuf} has
     *                                  not at least
     *                                  {@link #SSL_RECORD_HEADER_LENGTH}
     *                                  bytes to read.
     */
    static int getEncryptedPacketLength(ByteBuf buffer, int offset) {
        ...
        if (!tls) {
            ...
            if (majorVersion == 2 || majorVersion == 3) {
                // SSLv2
                packetLength = headerLength == 2 ? (shortBE(buffer, offset) & 0x7FFF) + 2
                        : (shortBE(buffer, offset) & 0x3FFF) + 3;
    ...
    private static short shortBE(ByteBuffer buffer, int offset) {
        return buffer.order() == ByteOrder.BIG_ENDIAN ? buffer.getShort(offset)
                : ByteBufUtil.swapShort(buffer.getShort(offset));
    }

    static int getEncryptedPacketLength(ByteBuffer[] buffers, int offset) {
        ByteBuffer buffer = buffers[offset];

        // Check if everything we need is in one ByteBuffer. If so we can make use of
        // the fast-path.
        if (buffer.remaining() >= SSL_RECORD_HEADER_LENGTH) {
            return getEncryptedPacketLength(buffer);
        }

        // We need to copy 5 bytes into a temporary buffer so we can parse out the
        // packet length easily.
    ...
    private static int getEncryptedPacketLength(ByteBuffer buffer) {
        ...
        if (!tls) {
            ...
            if (majorVersion == 2 || majorVersion == 3) {
                // SSLv2
                packetLength = headerLength == 2 ? (shortBE(buffer, pos) & 0x7FFF) + 2
                        : (shortBE(buffer, pos) & 0x3FFF) + 3;
                if (packetLength <= headerLength) {
                    return NOT_ENOUGH_DATA;
                }
            } else {
    ...
    static void handleHandshakeFailure(ChannelHandlerContext ctx, Throwable cause, boolean notify) {
        // We have may haven written some parts of data before an exception was thrown
        // so ensure we always flush.
    ...
    /**
     * Same as {@link Base64#encode(ByteBuf, boolean)} but allows the use of a
     * custom {@link ByteBufAllocator}.
     *
    ...
    /**
     * Returns {@code true} if the given cipher (in openssl format) is for TLSv1.3,
