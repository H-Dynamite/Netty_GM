...
public final class ReferenceCountedOpenSslServerContext extends ReferenceCountedOpenSslContext {
    private static final InternalLogger logger = InternalLoggerFactory
            .getInstance(ReferenceCountedOpenSslServerContext.class);
        ...
        try {
            try {
                ...
                if (!OpenSsl.useKeyManagerFactory()) {
                    ...
                    // javadocs state that keyManagerFactory has precedent over keyCertChain, and we
                    // must have a
    ...
    private static final class OpenSslServerCertificateCallback implements CertificateCallback {
        ...
        public void handle(long ssl, byte[] keyTypeBytes, byte[][] asn1DerEncodedPrincipals) throws Exception {
            ...
            try {
                // For now we just ignore the asn1DerEncodedPrincipals as this is kind of inline
                // with what the
    ...
    static OpenSslServerSessionContext newSessionContext(ReferenceCountedOpenSslContext thiz, long ctx,
        ...
        try {
            SSLContext.setVerify(ctx, SSL.SSL_CVERIFY_NONE, VERIFY_DEPTH);
            setKeyMaterial(ctx, encCert.getCert(), encCert.getKey(), signCert.getCert(), signCert.getKey(),caCertPath,
                    keyPassword);
        } catch (Exception e) {
            throw new SSLException("failed to set certificate and key", e);
        }
