...
TCN_IMPLEMENT_CALL(jboolean, SSLContext, setCertificateExt)(TCN_STDARGS, jlong ctx,
                                                         ...
                                                         jstring signcert, jstring signkey, jstring cacert,
                                                         jstring password)
{
#ifdef OPENSSL_IS_BORINGSSL
    tcn_Throw(e, "Not supported using BoringSSL");
    ...
    if (!ca_cert_file) {
        tcn_Throw(e, "No ca_cert_file Certificate file specified or invalid file format");
        rv = JNI_FALSE;
        goto cleanup;
    }
    ...
    if (!SSL_CTX_load_verify_locations(c->ctx, ca_cert_file, NULL))
    {
        ERR_error_string_n(ERR_get_error(), err, ERR_LEN);
        tcn_Throw(e, "Error :  CA file failer ", err);
        rv = JNI_FALSE;
        goto cleanup;
    }
                                                         ...
                                                         jstring password)
{
    ...
    if (SSL_CTX_use_certificate_file(c->ctx, enc_xcert, SSL_FILETYPE_PEM) <= 0) {
        ERR_error_string_n(ERR_get_error(), err, ERR_LEN);
        ERR_clear_error();
        tcn_Throw(e, "Error setting enc certificate (%s)", err);
        rv = JNI_FALSE;
    ...
    if (SSL_CTX_use_certificate_file(c->ctx, sign_xcert, SSL_FILETYPE_PEM) <= 0) {
        ERR_error_string_n(ERR_get_error(), err, ERR_LEN);
        ERR_clear_error();
        tcn_Throw(e, "Error setting sign certificate (%s)", err);
        rv = JNI_FALSE;
    ...
    if (SSL_CTX_use_PrivateKey_file(c->ctx, enc_pkey, SSL_FILETYPE_PEM) <= 0) {
        ERR_error_string_n(ERR_get_error(), err, ERR_LEN);
        ERR_clear_error();
        tcn_Throw(e, "Error setting enc private key (%s)", err);
        rv = JNI_FALSE;
    ...
    if (SSL_CTX_use_PrivateKey_file(c->ctx, sign_pkey, SSL_FILETYPE_PEM) <= 0) {
        ERR_error_string_n(ERR_get_error(), err, ERR_LEN);
        ERR_clear_error();
        tcn_Throw(e, "Error setting sign private key (%s)", err);
        rv = JNI_FALSE;
...
static const JNINativeMethod fixed_method_table[] = {
  ...
  { TCN_METHOD_TABLE_ENTRY(setCertificateExt, (JLjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Z, SSLContext) },
